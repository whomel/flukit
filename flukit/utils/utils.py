...
def load_features(reference, feature_names=None):
    ...
    for feat in SeqIO.read(reference, "genbank").features:
        if feat.type == "CDS":
            ...
            feat.type == "source"
        ):  # read 'nuc' as well for annotations - need start/end of whole!
            features["nuc"] = feat
...
def safe_translate(sequence, report_exceptions=False):
    ...
    try:
        ...
        codons = np.frombuffer(
            str_seq[: len(str_seq) - len(str_seq) % 3].encode(), dtype="S3"
        ).astype("U")
        assert len(codons) > 0
        aas = []
...
def get_reference(input_lineage: str, input_gene: str) -> SeqRecord:
    ...
    if input_lineage.lower() in lineages and input_gene.lower() in genes:
        refname = (
            f"{config_path}/reference_{input_lineage.lower()}_{input_gene.lower()}.gb"
        )
        try:
            ref = SeqIO.read(refname, "genbank")
        except Exception as e:
            raise ValueError(
                "Error: %s. While reading reference, check file is genbank format and it exists"
                % e
            )
    else:
        raise ValueError(
...
def locate_fasta(dir: Path, match: list, batch_num: str = None) -> list:
    ...
    if batch_num:
        files = [
            file for file in dir.glob("*.fasta") if re.search(batch_num, file.name)
        ]
